...
void Java_com_camera_simpledoublewebcams2_CameraPreview_pixeltobmp( JNIEnv* env,jclass thiz,jobject bitmap1, jobject bitmap2){
...
void Java_com_camera_simpledoublewebcams2_CameraPreview_pixeltobmp4( JNIEnv* env,jclass thiz,jobject bitmap1, jobject bitmap2, jobject bitmap3, jobject bitmap4){
	...
	for(j=0 ; j<CAMERANUM ; j++){
		...
		if(j==0){
			...
			if ((ret = AndroidBitmap_lockPixels(env, bitmap2, &pixels)) < 0) {
				LOGE("AndroidBitmap_lockPixels() failed ! error=%d", ret);
					...
					if ((ret = AndroidBitmap_getInfo(env, bitmap3, &info)) < 0) {
						LOGE("AndroidBitmap_getInfo() failed ! error=%d", ret);
						return;
					}
					...
					if ((ret = AndroidBitmap_lockPixels(env, bitmap3, &pixels)) < 0) {
						LOGE("AndroidBitmap_lockPixels() failed ! error=%d", ret);
					}
				}

		else if(j==3){
					if(bitmap4==NULL) continue;
					if ((ret = AndroidBitmap_getInfo(env, bitmap4, &info)) < 0) {
						LOGE("AndroidBitmap_getInfo() failed ! error=%d", ret);
						return;
					}
					...
					if ((ret = AndroidBitmap_lockPixels(env, bitmap4, &pixels)) < 0) {
						LOGE("AndroidBitmap_lockPixels() failed ! error=%d", ret);
					}
				}
		...
		if(j==0){
			...
			AndroidBitmap_unlockPixels(env, bitmap3);
		}else if(j==3){
			AndroidBitmap_unlockPixels(env, bitmap4);
		}
	}
}
jint Java_com_camera_simpledoublewebcams2_CameraPreview_prepareCamera(JNIEnv* env,jclass thiz, jint videoid){
	...
	if(ret[0]==ERROR_LOCAL || ret[1]==ERROR_LOCAL ){
		return ret[1]*8+ret[1]*4+ret[1]*2+ret[0];
	}else{
		return SUCCESS_LOCAL ;
	}
}	

jint Java_com_camera_simpledoublewebcams2_CameraPreview_prepareCameraWithBase( JNIEnv* env,jclass thiz, jint videoid, jint videobase){
...
void Java_com_camera_simpledoublewebcams2_CameraPreview_processCamera( JNIEnv* env,jclass thiz){
...
void Java_com_camera_simpledoublewebcams2_CameraPreview_stopCamera(JNIEnv* env,jclass thiz){
...
void Java_com_camera_simpledoublewebcams2_CameraPreview_processRBCamera( JNIEnv* env,jclass thiz, jint lrmode){
	int i;
	int red = 1-lrmode;
	int blue = lrmode;
